...
<body id="page-top">
    ...
    <div class="container-fluid p-0">
        ...
        <!-- compétences-->
        <?php
        // get info compétences
        $assoComeptence = $bdd->prepare("SELECT * FROM asso_competence_profile  WHERE id_profile = " . $idProfile);
        $assoComeptence->execute();
        $competences = new ArrayObject();
        while ($row = $assoComeptence->fetch(PDO::FETCH_OBJ)) {
            $comeptence = $bdd->prepare("SELECT * FROM competence WHERE id = " . $row->id_competence);
            $comeptence->execute();
            $resultCompetence = $comeptence->fetch(PDO::FETCH_OBJ);
            $competences->append($resultCompetence);
        }
        //
        ?>
